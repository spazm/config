...
if ( $setup )
 then
	printf "Performing software updates/installs to make sure you have everything we need, then we'll get started.\n"
		...
		sudo apt-get -y install php php-xml php-mbstring php-common php-cli wget  1>$logs/setup1.log 2>&1
		sudo wget -qO- https://deb.nodesource.com/setup_4.x | bash - 1>$logs/setup2.log 2>&1
		sudo apt-get -y install nodejs 1>$logs/setup3.log 2>&1
		sudo wget -qO- https://deb.nodesource.com/setup_6.x | bash - 1>$logs/setup4.log 2>&1
		sudo apt-get -y install npm 1>$logs/setup5.log 2>&1
		sudo npm install npm -g 1>$logs/setup6.log 2>&1
		sudo npm install slack-history-export -g 1>$logs/setup7.log 2>&1
	printf "\n"
	exit 200
fi
...
printf "Checking Slack Token for valid auth..."
	wget --post-data="token=$slack_token" https://slack.com/api/auth.test -O $debug/auth.test  1>$logs/wget_auth.log 2>&1
	echo "auth.test:"
	cat $debug/auth.test
	echo "auth logs:"
	cat $logs/wget_auth.log
...
#Get a json response from the slack API then clean it up; remove the first X lines of the responses that show: 
	...
	#}
#params from all files created as a result of the wget api calls.
printf "Getting Channel meta data..."
	#wget --post-data="token=$slack_token" https://slack.com/api/channels.list -O "channels.list.json" 1>$logs/wget_meta01.log 2>&1
	wget --post-data="token=$slack_token" https://slack.com/api/conversations.list -O "channels.list.json" 1>$logs/wget_meta01.log 2>&1
...
printf "Getting Users meta data..."
	wget --post-data="token=$slack_token" https://slack.com/api/users.list -O "users.list.json" 1>$logs/wget_meta02.log 2>&1
...
printf "Getting IntegrationLogs data..."
	wget --post-data="token=$slack_token" https://slack.com/api/team.integrationLogs.list -O "team.integrationLogs.json" 1>$logs/wget_meta03.log 2>&1
...
printf "Getting additional meta data..."
	wget --post-data="token=$slack_token" https://slack.com/api/team.info -O "$debug/team.info.json" 1>$logs/wget_meta04.log 2>&1
	wget --post-data="token=$slack_token" https://slack.com/api/reminders.list -O "$debug/reminders.list.json" 1>$logs/wget_meta05.log 2>&1
	wget --post-data="token=$slack_token" https://slack.com/api/emoji.list -O "$debug/emoji.list.json" 1>$logs/wget_meta06.log 2>&1
	wget --post-data="token=$slack_token" https://slack.com/api/users.conversations -O "$debug/im.list.json" 1>$logs/wget_meta07.log 2>&1
	wget --post-data="token=$slack_token" https://slack.com/api/conversations.list -O "$debug/groups.list.json" 1>$logs/wget_meta08.log 2>&1
	wget --post-data="token=$slack_token" https://slack.com/api/conversations.list -O "$debug/channels.list.json" 1>$logs/wget_meta09.log 2>&1
	wget --post-data="token=$slack_token" https://slack.com/api/mpim.list -O "$debug/mpim.list.json" 1>$logs/wget_meta10.log 2>&1 #slack-history-export can't handle these yet
printf "done.\n"
##################################
		...
		if ( $fetch_users )
		 then
			...
			for user in "${user_list[@]}"
			 do
				wget --post-data="token=$slack_token" "https://slack.com/api/users.info&user=$user" -O "$debug/users/$user.json" 1>$logs/users/$user.log 2>&1
